...
```
1. 确认优化范围：
   - 全部skills → 扫描以下路径（按优先级）：
     a. 当前 workspace: .claude/skills/*/SKILL.md
     b. 全局 agents 目录: .agents/skills/*/SKILL.md
     c. 其他已授权的 working directory 下的 .claude/skills/*/SKILL.md
   - 指定skills → 用户指定列表
...
**skill 定位规则**：当用户说"优化 XXX skill"时，按以下顺序定位：

1. 在当前 workspace `.claude/skills/XXX/` 查找 SKILL.md
2. 在 `.agents/skills/XXX/` 查找 SKILL.md
3. 在 VSCode/Claude 扩展返回的可用 skill 列表中查找 XXX
4. 如果以上都没有 → 告知用户具体诊断结果，不要猜测

**重要区分**：
- **本地 skill**：目录 + SKILL.md 文件存在 → 可直接优化
- **远程/marketplace skill**：扩展列表里有，但本地无 SKILL.md → 无法直接优化；可帮用户基于描述新建，或让用户先复制到本地
- **未安装的 skill**：列表里也没有 → 需要用户确认是否要创建
...
| SKILL.md 找不到（目录存在） | 目录存在但无 SKILL.md | 该 skill 终止，results.tsv 记 `status=error`，继续下一个 |
| **Skill 列表有但本地无文件** | 扩展/marketplace 返回 skill 名，但 `.claude/skills/` 和 `.agents/skills/` 都找不到 SKILL.md | **先诊断**：列出已扫描路径 + 确认文件确实不存在；**再给用户 3 个选项**：① 基于描述新建本地 SKILL.md ② 让用户把远程 skill 复制到本地 ③ 跳过该 skill 优化下一个 |
| **用户指定 skill 找不到** | 用户说"优化 XXX"，但所有扫描路径都没有 | 不要直接说"不能编辑"；先报告扫描结果，再问：① 名字是否记错 ② 是否要创建 ③ 是否在其他目录 |
