...
import { layoutSemanticFamily } from './workflow/layout-query.mjs';
import { sha256File } from './workflow/artifact-chain.mjs';
import {
...
const SOURCE_CLAIM_STATUSES = new Set(['approved', 'blocked', 'excluded', 'pending']);
const PRESENTATION_FIELD_REF_SOURCES = Object.freeze({
  title: ['title', 'titleShort'],
  summary: ['summary', 'summaryShort'],
  takeaway: ['takeaway'],
});
const PRESENTATION_STRUCTURES = new Set([
  'single',
  'parallel',
  'sequence',
  'comparison',
...
export function validateGoalSpec(spec, options = {}) {
  ...
  pushUniqueErrors(errors, validateGoalQualityContract(spec, options));
...
/**
 * Deterministic fact, page-duty, structure, and capacity gates for goals that
 * explicitly opt into provenance.  Goals authored before provenance existed
 * remain byte-for-byte compatible because the gate is inactive until either a
 * deck sourceLedger or a slide content.provenance object is present.
 */
export function validateGoalQualityContract(spec, options = {}) {
  const errors = [];
  const slides = Array.isArray(spec?.slides) ? spec.slides : [];
  if (spec?.artifactContract != null) {
    if (!isPlainRecord(spec.artifactContract)
      || spec.artifactContract.version !== 1
      || spec.artifactContract.required !== true) {
      errors.push('deck field artifactContract: expected {version:1, required:true}');
    }
    if (!String(spec?.workflowRunId || '').trim()) {
      errors.push('deck field workflowRunId: required by artifactContract version 1');
    }
  }
  const hasSourceLedger = Object.prototype.hasOwnProperty.call(spec || {}, 'sourceLedger');
  const hasSlideProvenance = slides.some(slide => isPlainRecord(slide?.content?.provenance));
  const declaredContentMode = String(spec?.contentMode || '').trim().toLowerCase();
  const sourceBoundRequired = options.requireSourceBound === true
    || declaredContentMode === 'source-bound';
  const provenanceEnabled = sourceBoundRequired || hasSourceLedger || hasSlideProvenance;

  if (sourceBoundRequired && declaredContentMode !== 'source-bound') {
    errors.push('deck field contentMode: "source-bound" is required by this workflow');
  }
  if (declaredContentMode && !['creative', 'source-bound'].includes(declaredContentMode)) {
    errors.push(`deck field contentMode: unsupported "${spec.contentMode}"`);
  }
  const declaredDesignMode = String(spec?.designMode || '').trim().toLowerCase();
  if (declaredDesignMode && !['preset', 'reference-bound'].includes(declaredDesignMode)) {
    errors.push(`deck field designMode: unsupported "${spec.designMode}"`);
  }
  if (declaredContentMode === 'creative' && (hasSourceLedger || hasSlideProvenance)) {
    errors.push('deck field contentMode: creative cannot be used with sourceLedger or slide provenance; use "source-bound"');
  }

  validateReferenceBinding(spec, options, errors);
  if (!provenanceEnabled) return errors;

  const slideIds = new Set(slides.map((slide, index) => resolveLogicalSlideId(slide, index)));
  const claimsById = validateSourceLedger(
    spec?.sourceLedger,
    slideIds,
    errors,
    { allowExternalOwners: isPlainRecord(spec?.part) },
  );
  const questionUsages = new Map();
  const claimSlideUsages = new Map();

  slides.forEach((slide, index) => {
    const slideNumber = index + 1;
    const slideId = resolveLogicalSlideId(slide, index);
    const content = isPlainRecord(slide?.content) ? slide.content : {};
    const presentation = isPlainRecord(content.presentation) ? content.presentation : {};
    const provenance = content.provenance;
    const referencedClaimIds = new Set();

    let pageClaimIds = new Set();
    if (!isPlainRecord(provenance)) {
      errors.push(`slide ${slideNumber} field content.provenance: object is required when source provenance is enabled`);
    } else {
      const pageQuestion = String(provenance.pageQuestion || '').trim();
      const sectionId = String(provenance.sectionId || '').trim();
      if (!pageQuestion) {
        errors.push(`slide ${slideNumber} field content.provenance.pageQuestion: non-empty page responsibility is required`);
      } else {
        const key = normalizePageQuestion(pageQuestion);
        const usages = questionUsages.get(key) || [];
        usages.push({ slideNumber, pageQuestion });
        questionUsages.set(key, usages);
      }
      if (!sectionId) {
        errors.push(`slide ${slideNumber} field content.provenance.sectionId: non-empty section id is required`);
      }
      addClaimIds(
        provenance.claimIds,
        `slide ${slideNumber} field content.provenance.claimIds`,
        referencedClaimIds,
        errors,
        { required: true },
      );
      pageClaimIds = new Set(referencedClaimIds);
      if (provenance.fieldRefs != null) {
        if (!isPlainRecord(provenance.fieldRefs)) {
          errors.push(`slide ${slideNumber} field content.provenance.fieldRefs: expected an object of claim-id arrays`);
        }
      }
      validatePresentationFieldRefs(
        presentation,
        provenance.fieldRefs,
        pageClaimIds,
        referencedClaimIds,
        slideNumber,
        errors,
    ...
    const structure = String(presentation.structure || '').trim().toLowerCase();
    if (!structure) {
      errors.push(`slide ${slideNumber} field content.presentation.structure: required when source provenance is enabled`);
    } else if (!PRESENTATION_STRUCTURES.has(structure)) {
      errors.push(`slide ${slideNumber} field content.presentation.structure: unsupported "${presentation.structure}"`);
    }

    validatePresentationFactCapacity(
      presentation,
      slideNumber,
      referencedClaimIds,
      errors,
    );
    collectOptionalClaimIds(
      presentation.chartData,
      `slide ${slideNumber} field content.presentation.chartData`,
      referencedClaimIds,
      errors,
    );
    ...
    for (const claimId of referencedClaimIds) {
      ...
      if (!claim) {
        errors.push(`slide ${slideNumber} field provenance: unknown source claim "${claimId}"`);
        continue;
      }
      if (claim.status !== 'approved') {
        errors.push(`slide ${slideNumber} field provenance: source claim "${claimId}" is ${claim.status} and cannot be presented`);
      }
      if (claim.ownerSlideId && claim.ownerSlideId !== slideId && claim.allowReuse !== true) {
        errors.push(
          `slide ${slideNumber} field provenance: source claim "${claimId}" belongs to slide "${claim.ownerSlideId}"; set allowReuse=true for authorized cross-page reuse`,
        );
      }
      const usages = claimSlideUsages.get(claimId) || new Set();
      usages.add(slideId);
      claimSlideUsages.set(claimId, usages);
    }

    validateExcludedTerms(slide, content, claimsById, slideNumber, errors);
    validatePresentationStructureVariants(slide, presentation, structure, slideNumber, errors);
  });

  for (const usages of questionUsages.values()) {
    if (usages.length <= 1) continue;
    errors.push(
      `deck field page responsibilities: duplicate pageQuestion "${usages[0].pageQuestion}" on slides ${usages.map(item => item.slideNumber).join(', ')}`,
    );
  }

  for (const [claimId, usages] of claimSlideUsages.entries()) {
    const claim = claimsById.get(claimId);
    if (usages.size <= 1 || claim?.allowReuse === true) continue;
    errors.push(
      `deck field sourceLedger: source claim "${claimId}" is reused across slides ${[...usages].join(', ')} without allowReuse=true`,
    );
  }
...
function validateReferenceBinding(spec, options, errors) {
  const required = options.requireReferenceBound === true
    || String(spec?.designMode || '').trim().toLowerCase() === 'reference-bound'
    || Object.prototype.hasOwnProperty.call(spec || {}, 'referenceBinding');
  if (!required) return;
  if (String(spec?.designMode || '').trim().toLowerCase() !== 'reference-bound') {
    errors.push('deck field designMode: "reference-bound" is required by this workflow');
  }
  if (!isPlainRecord(spec?.referenceBinding)) {
    errors.push('deck field referenceBinding: object is required for a binding reference task');
    return;
  }
  if (String(spec.referenceBinding.supportStatus || '').trim().toLowerCase() !== 'supported') {
    errors.push('deck field referenceBinding.supportStatus: binding reference runtime is not supported; stop instead of using an approximate theme');
  }
  // Dashi 0.4.12 only registers preset theme01-theme12. Until an external
  // reference runtime is registered, a preset theme may not impersonate one.
  errors.push('deck field referenceBinding: external binding reference themes are not registered in this Dashi runtime; generation is blocked');
}

function validateSourceLedger(sourceLedger, slideIds, errors, options = {}) {
  const claimsById = new Map();
  if (!isPlainRecord(sourceLedger)) {
    errors.push('deck field sourceLedger: object is required when source provenance is enabled');
    return claimsById;
  }
  if (!Array.isArray(sourceLedger.claims) || !sourceLedger.claims.length) {
    errors.push('deck field sourceLedger.claims: non-empty array is required');
    return claimsById;
  }
  sourceLedger.claims.forEach((claim, index) => {
    const pathName = `deck field sourceLedger.claims[${index}]`;
    ...
    if (!id) {
      errors.push(`${pathName}.id: non-empty claim id is required`);
      return;
    }
    if (claimsById.has(id)) {
      errors.push(`${pathName}.id: duplicate source claim "${id}"`);
      return;
    }
    if (!SOURCE_CLAIM_STATUSES.has(status)) {
      errors.push(`${pathName}.status: unsupported "${claim.status ?? ''}"`);
    }
    const locator = String(claim.locator || '').trim();
    if (!locator) {
      errors.push(`${pathName}.locator: non-empty source locator is required for claim "${id}"`);
    }
    if (claim.allowReuse != null && typeof claim.allowReuse !== 'boolean') {
      errors.push(`${pathName}.allowReuse: expected a boolean`);
    }
    const ownerSlideId = String(claim.ownerSlideId || '').trim();
    if (ownerSlideId && !slideIds.has(ownerSlideId) && !options.allowExternalOwners) {
      errors.push(`${pathName}.ownerSlideId: unknown logical slide "${ownerSlideId}"`);
    }
    if (claim.terms != null) {
      if (!Array.isArray(claim.terms) || claim.terms.some(term => !String(term || '').trim())) {
        errors.push(`${pathName}.terms: expected an array of non-empty strings`);
      }
    }
    if (['blocked', 'excluded'].includes(status)
      && (!Array.isArray(claim.terms) || !claim.terms.some(term => String(term || '').trim()))) {
      errors.push(`${pathName}.terms: blocked and excluded claims require at least one denied term`);
    }
    if (status === 'approved' && !ownerSlideId) {
      errors.push(`${pathName}.ownerSlideId: approved claims require one default owner slide`);
    }
    claimsById.set(id, {
      ...claim,
      id,
      status,
      ...(locator ? { locator } : {}),
      ...(ownerSlideId ? { ownerSlideId } : {}),
    });
  });
  return claimsById;
}

function addClaimIds(value, pathName, destination, errors, options = {}) {
  if (value == null) {
    if (options.required) errors.push(`${pathName}: array is required`);
    return;
  }
  if (!Array.isArray(value)) {
    errors.push(`${pathName}: expected an array of non-empty claim ids`);
    return;
  }
  if (options.required && value.length === 0) {
    errors.push(`${pathName}: at least one source claim id is required`);
  }
  const local = new Set();
  value.forEach((rawId, index) => {
    const id = String(rawId || '').trim();
    if (!id) {
      errors.push(`${pathName}[${index}]: non-empty claim id is required`);
      return;
    }
    if (local.has(id)) errors.push(`${pathName}: duplicate claim id "${id}"`);
    local.add(id);
    destination.add(id);
  });
}

function collectOptionalClaimIds(items, pathName, destination, errors) {
  if (!Array.isArray(items)) return;
  items.forEach((item, index) => {
    if (!isPlainRecord(item) || item.claimIds == null) return;
    addClaimIds(item.claimIds, `${pathName}[${index}].claimIds`, destination, errors);
  });
}

function validatePresentationFactCapacity(presentation, slideNumber, destination, errors) {
  if (!Array.isArray(presentation.items) || !presentation.items.length) return;
  const visibleFingerprints = new Map();
  presentation.items.forEach((item, index) => {
    const pathName = `slide ${slideNumber} field content.presentation.items[${index}].claimIds`;
    if (!isPlainRecord(item)) {
      errors.push(`slide ${slideNumber} field content.presentation.items[${index}]: expected an object with claimIds`);
      return;
    }
    addClaimIds(item.claimIds, pathName, destination, errors, { required: true });
    const fingerprint = visibleItemFingerprint(item);
    if (!fingerprint) return;
    const prior = visibleFingerprints.get(fingerprint);
    if (prior != null) {
      errors.push(`slide ${slideNumber} field content.presentation.items: repeated visible fact text on items ${prior + 1} and ${index + 1}`);
    } else {
      visibleFingerprints.set(fingerprint, index);
    }
  });
}

function visibleItemFingerprint(item) {
  const visible = Object.fromEntries(Object.entries(item || {})
    .filter(([key, value]) => !['id', 'claimIds', 'required', 'priority'].includes(key) && value != null && value !== ''));
  return JSON.stringify(visible).toLocaleLowerCase().replace(/\s+/g, '');
}

function validatePresentationFieldRefs(presentation, fieldRefs, pageClaimIds, destination, slideNumber, errors) {
  const refs = isPlainRecord(fieldRefs) ? fieldRefs : {};
  const allowedFields = Object.keys(PRESENTATION_FIELD_REF_SOURCES);

  for (const [field, claimIds] of Object.entries(refs)) {
    const pathName = `slide ${slideNumber} field content.provenance.fieldRefs.${field}`;
    const ids = new Set();
    addClaimIds(claimIds, pathName, ids, errors, { required: true });
    for (const id of ids) {
      destination.add(id);
      if (!pageClaimIds.has(id)) {
        errors.push(`${pathName}: claim "${id}" must also appear in provenance.claimIds`);
      }
    }

    const sourceFields = PRESENTATION_FIELD_REF_SOURCES[field];
    if (!sourceFields) {
      errors.push(`${pathName}: unsupported field reference; allowed fields are ${allowedFields.join(', ')}`);
      continue;
    }
    if (!sourceFields.some(sourceField => hasVisibleNarrativeValue(presentation[sourceField]))) {
      errors.push(`${pathName}: field reference does not correspond to a visible presentation.${sourceFields.join(' or presentation.')}`);
    }
  }

  for (const [field, sourceFields] of Object.entries(PRESENTATION_FIELD_REF_SOURCES)) {
    if (!sourceFields.some(sourceField => hasVisibleNarrativeValue(presentation[sourceField]))) continue;
    if (Object.prototype.hasOwnProperty.call(refs, field)) continue;
    addClaimIds(
      undefined,
      `slide ${slideNumber} field content.provenance.fieldRefs.${field}`,
      new Set(),
      errors,
      { required: true },
    );
...
function validateExcludedTerms(slide, content, claimsById, slideNumber, errors) {
  const searchable = JSON.stringify(materializedVisibleContent(slide, content)).toLocaleLowerCase();
  for (const claim of claimsById.values()) {
    if (claim.status === 'approved' || !Array.isArray(claim.terms)) continue;
    for (const rawTerm of claim.terms) {
      const term = String(rawTerm || '').trim().toLocaleLowerCase();
      if (!term || !searchable.includes(term)) continue;
      errors.push(
        `slide ${slideNumber} field content: contains ${claim.status} source term "${rawTerm}" from claim "${claim.id}"`,
      );
    }
  }
}

function materializedVisibleContent(slide, content) {
  const values = [content?.presentation || {}];
  for (const variant of slide?.variants || []) {
    const base = variant?.kind === 'bespoke' ? variant?.composition : variant?.props;
    let resolved = isPlainRecord(base) ? base : {};
    try {
      if (isPlainRecord(variant?.contentMap)) {
        resolved = resolveContentMap(content, variant.contentMap, resolved);
      }
    } catch {
      // The main validator reports invalid contentMap paths separately.
    }
    if (variant?.kind === 'bespoke') {
      const targets = bespokeContentTargets(resolved).visibleTargets;
      values.push(targets.map(target => readSimplePath(resolved, target).value));
    } else if (variant?.layout && layoutExists(variant.layout)) {
      const inspected = inspectLayout(variant.layout, { compact: true }) || {};
      values.push(visiblePropsForInspection(resolved, inspected, variant?.props || {}));
    } else {
...
function validatePresentationStructureVariants(slide, presentation, structure, slideNumber, errors) {
  if (structure !== 'parallel' || !Array.isArray(slide?.variants)) return;
  const contentShape = contentShapeFromPresentation(presentation);
  slide.variants.forEach((variant, index) => {
    const variantId = String(variant?.id || index + 1);
    if (variant?.kind === 'bespoke') {
      const family = String(variant?.composition?.designIntent?.compositionFamily || '').trim().toLowerCase();
      if (family === 'timeline' || family === 'process' || family === 'sequence') {
        errors.push(
          `slide ${slideNumber} variant ${variantId} field composition.designIntent.compositionFamily: "${family}" contradicts presentation.structure "parallel"`,
        );
      }
      return;
    }
    if (!variant?.layout || !layoutExists(variant.layout)) return;
    const inspected = inspectLayout(variant.layout, { compact: true }) || {};
    const projectionPlan = buildTemplateProjectionPlan(inspected, contentShape);
    const family = layoutSemanticFamily(inspected, projectionPlan);
    if (family === 'sequence') {
      errors.push(
        `slide ${slideNumber} variant ${variantId} field layout: semantic family "sequence" contradicts presentation.structure "parallel"`,
      );
    }
  });
}

function normalizePageQuestion(value) {
...
function runCli() {
  ...
  try {
    ...
    const errors = validateGoalSpec(spec, {
      requireSourceBound: parsed.requireSourceBound,
      requireReferenceBound: parsed.requireReferenceBound,
    });
    if (errors.length) {
      const error = new Error(errors.join('; '));
      telemetry.finish({ ok: false, error, metrics: { goalSha256 } });
      console.error('Goal spec validation failed:');
      for (const validationError of errors) console.error(`- ${validationError}`);
      process.exitCode = 1;
      return;
    }
    telemetry.finish({ ok: true, metrics: { goalSha256 } });
...
function parseCliArgs(argv) {
  const parsed = {
    file: null,
    requireSourceBound: false,
    requireReferenceBound: false,
  };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--require-reference-bound') {
  ...
  return parsed.file ? parsed : { error: 'Missing goal spec path.' };
}
