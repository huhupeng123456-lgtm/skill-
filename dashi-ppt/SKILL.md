---
name: dashi-ppt
description: 从零制作 PPT、演示文稿、幻灯片和汇报材料时使用。Dashi PPT 基于预置视觉主题或用户提供的参考 PPT 设计新页面,生成可离线打开、可在浏览器编辑的 HTML 演示,支持导出 PPTX / PDF；有参考 PPT 时须继承其整体视觉语言,在 PPT 上下文中需要原创配图时须提供 3–5 个同规格候选供用户选择。
---
...
用户提供参考 PPT、现有模板或完整演示稿,或要求“沿用原风格”“参考这个 PPT”“不要改图片位置/页面结构”时,必须先完整阅读 [references/reference-deck-workflow.md](references/reference-deck-workflow.md)。

在 PPT 制作上下文中需要生成原创配图时,先按最终图片槽确定比例和构图安全区,默认生成 3 个独立候选,封面、章节页和关键转折页可生成 5 个；候选保持相同比例、主题和整体画风,逐张提供而非合成拼图,待用户选择后再写入正式 deck。只有用户明确表示“不用选,直接定”时才跳过选择。

## 四项硬门禁

1. **事实与用户否定**:用户把 Word、PDF、PPT、表格或知识库作为内容来源,或明确要求“不得编造”时,按 source-bound 任务处理；仅用于视觉参考的文件不自动约束内容。只能压缩、重组已确认事实；用户明确否定的内容优先级高于源文本,即使源文本出现也不得继续使用。冲突未解决时标记待确认或阻塞,不自行选边。
2. **参考稿绑定**:有参考稿时,其背景、标题体系、字体层级、网格、页面密度和装饰语言是绑定设计系统。不得改用相近内置主题或手工近似稿代替；Dashi 链路不支持时必须停止并标记“阻塞”。
3. **事实容量与页面结构**:页数由可用事实和阅读容量决定,不为凑页重复标题、副标题、结论或同一组事实。并列内容使用分栏、矩阵或平铺,只有真实的时间、步骤或因果顺序才使用时间线/流程；信息不足时合并或减少页面。
4. **Dashi 产物身份**:只有由本次新建的 `goal.json` 经 Dashi 生成器渲染为 HTML,完成本 Skill 规定的校验,再通过 Dashi 导出链路产生的 HTML/PPTX/PDF,才能称为“使用 Dashi PPT 制作”。新稿必须具备 `artifactContract`、`dashi-build.json`、通过质量校验的同一组哈希,导出文件还须带 `.dashi-export.json`;缺失时只标记 `legacy-unverified`,不得冒充已验证的 Dashi 产物。直接用 `python-pptx`、Office COM、手写 HTML 或其他链路生成的文件不得冒充 Dashi 产物。这里的 `verified` 只表示复验当时本地成品、HTML、goal、构建记录、工作流记录和导出清单彼此一致,不是数字签名、可信时间戳或第三方证明；它不能对抗能同时修改程序源码和这些记录的恶意操作者,不得宣称绝对防伪。

source-bound 任务用 `goal:scaffold --source-bound` 创建并在 goal 顶层写 `contentMode:"source-bound"` 与 `sourceLedger.claims[]`;渲染包装脚本同时传 `-SourceBound`(PowerShell)或 `--source-bound`(shell),使外部工作流即使漏写来源字段也会阻塞。每条 claim 至少包含稳定 `id`、来源定位 `locator`、`approved/pending/blocked/excluded` 状态和默认唯一的 `ownerSlideId`;`blocked/excluded` 必须写非空 `terms`。每页写 `content.provenance.pageQuestion/sectionId/claimIds/fieldRefs`,标题、摘要、结论和可见 `items[]` 分别绑定 claim,并用 `presentation.structure` 标明 `single/parallel/sequence/comparison/hierarchy/relationship/timeline/process/matrix`。只有确需跨页复用时才对 claim 设置 `allowReuse: true`。字段契约以 [references/goal-spec.schema.json](references/goal-spec.schema.json) 为准。
...
- 开工前确认两件事:主题风格、是否需要图片/视频。用户未明确表达且非整体委托时,先提问等答复,不得代选;无法提问的环境(脚本/批处理)才自选,并在交付说明中列出所选与理由。用户已提供参考 PPT 时,参考稿本身就是主题风格,不再要求从内置主题中选择。
...
- PPTX 文件:仍先生成并校验 HTML。新 `artifactContract` 稿件使用 `npm run export:pptx` / `export:pdf` 的受验证链路导出并生成同名 `.dashi-export.json`;预览页的浏览器拼装兜底不能生成“已验证 Dashi”产物。交付前再运行 `npm --prefix <skill-root>/project run verify:export-manifest -- <deck-ppt-dir> <artifact.pptx-or-pdf>`,确认成品和 HTML 未被替换且整条本地记录链仍一致。最终只给 PPTX/PDF 文件路径或下载结果。
...
- 长 deck:先写逐页 brief JSON,每页包含 `role`、`priority` 和唯一的 `content`,再用 `npm --prefix <skill-root>/project run goal:scaffold -- --title <title> --goal <goal> --theme <themePack> --pages <n> --content-briefs <briefs.json> --layout-variants 3 --seed <randomSeed> --workflow-run-id <workflowRunId> --chunk-size 5 --out output/<deck-name>/goal.json`。有内容来源时追加 `--source-bound`;有绑定参考稿时追加 `--reference-bound`,当前运行时会明确阻塞而不是换近似主题。scaffold 按内容容量和字段能力选 3 个模板,并追加可直接渲染的主题化 v4 初稿。
...
1. 提炼用户目标: `title`、`goal`、`audience`、`owner`、页数、内容重点、完成状态边界和最终产物格式;同时形成验收清单,记录用户显式要求、已确认选项和必要假设。用户未指定页数时,按可用事实容量和阅读任务确定,不设凑页下限。
2. 用户提供参考 PPT 时,按参考工作流先形成视觉基因清单、源内容覆盖表和允许偏离项,把参考稿作为绑定设计系统,并以 `--reference-bound` 进入支持检查；当前 Dashi 未注册外部参考稿主题时必须阻塞。没有参考稿时再确认 `themePack`。用户未指定风格时先询问;用户选定后生成 `randomSeed` 和本次任务唯一的 `workflowRunId`,例如 `<主题>-<日期>-<3位随机词>`,保证随机选页可复现;同一任务重试复用该 run ID,新任务生成新 ID。
3. 判断图片意图:无图但需要视觉素材时先问是否预留图片槽;用户给本地素材先 `media:stage`;明确生图时先按最终槽位生成 3–5 个同规格候选,待用户选择或授权代选后再写入 deck。
...
6. 把 schema v2 JSON 写入本次工作目录的 `output/<deck-name>/goal.json`;新稿由 scaffold 自动写 `artifactContract:{version:1,required:true}`、`contentMode` 和 `designMode`,每页写 `selectedVariant: "v1"` 和恰好 3 template + 1 bespoke。source-bound 先运行带 `--require-source-bound` 的 goal spec 校验,再运行 `props:safe --write`;`--write` 后核对 3 个模板方案的 `layoutChanges`,不认可替换就改回并换页。
7. 图表页填入自己的数据后,页内 insight/读图/结论类文案字段必须据新数据一并改写,不保留默认结论。
8. 运行渲染脚本输出 `output/<deck-name>/ppt/index.html`;source-bound 传 `-SourceBound` / `--source-bound`,参考稿任务传 `-ReferenceBound` / `--reference-bound`。脚本会使用 Skill 内置生成器并写 `dashi-build.json`,不要切回外部项目目录。
...
15. 验收通过后按交付格式回复:HTML 只给 `http://127.0.0.1:<port>/`;PPTX/PDF 用受验证 CLI 导出后,立即运行 `verify:export-manifest` 独立复验当前 PPTX/PDF、HTML、build manifest、goal、telemetry 和 sidecar,命令成功后才给成品文件路径或下载结果。该复验是本地工作流一致性检查,不是加密签名或外部可信证明。
...
- 参考稿继承:存在参考 PPT 时,并排检查背景、配色、字体层级、标题体系、页边距、网格、页面密度、图片框和装饰语言；沿用参考结构的页面不擅自改变图片位置与视觉重心,新增结构看起来仍属于同一套演示稿。
- 事实与状态:主要章节有明确去向,没有把示例、规划或推断写成已完成成果,没有无来源数字或项目事实。
- 可编辑性:只有实际检查导出对象后才能声称表格、图表或其他元素原生可编辑；无法验证时明确说明,不能用文件后缀或视觉相似代替验收。
