...
export async function handleEditablePptxExport(req, res) {
  ...
  try {
    ...
    const chain = assertVerifiedApiExportReady(payload);
    ...
    writeApiExportManifest(chain, outFile, 'pptx');
...
export async function handlePdfExport(req, res) {
  ...
  try {
    ...
    writeApiExportManifest(chain, outFile, 'pdf');
    updateExportProgress(progressId, { stage: 'download-ready', detail: '准备浏览器下载', percent: 100, done: true });
...
function assertVerifiedApiExportReady(payload = {}) {
  const deckFile = path.join(SERVE_ROOT, 'index.html');
  const deckRoot = path.basename(SERVE_ROOT).toLowerCase() === 'ppt' ? path.dirname(SERVE_ROOT) : SERVE_ROOT;
  const goalPath = path.join(deckRoot, 'goal.json');
  const state = verifyBuildChain({
    goalPath: existsSync(goalPath) ? goalPath : null,
    deckFile,
    requireQuality: true,
  });
  if (state.strict && state.errors.length) {
    throw new Error(`Verified Dashi export blocked: ${state.errors.join('; ')}`);
  }
  if (state.strict && payload?.snapshot) {
    throw new Error('Verified Dashi export cannot use an unsaved browser snapshot; save, rerun validation, then export with the Dashi CLI');
  }
  return state;
}

function assertBrowserAssemblyAllowed() {
  const state = assertVerifiedApiExportReady();
  if (state.strict) {
    throw new Error('Verified Dashi export cannot use browser-assembled fallback bytes; use the Dashi CLI export so the input and output hashes can be verified');
  }
}

function writeApiExportManifest(state, outFile, format) {
  const exporterVersion = JSON.parse(readFileSync(path.join(ROOT, 'package.json'), 'utf8')).version;
  writeExportManifest({ state, artifactFile: outFile, format, exporterVersion });
}
