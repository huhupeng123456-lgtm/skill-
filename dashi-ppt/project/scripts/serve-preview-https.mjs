...
import { resolveOpenSslExecutablePath } from './openssl-path.mjs';
...
const HTTP_ONLY = process.env.DASHI_PPT_HTTP_ONLY === '1';
const READ_ONLY = process.env.DASHI_PPT_READ_ONLY === '1';
...
if (process.env.DASHI_PPT_SKIP_FRESHNESS !== '1') {
...
if (!HTTP_ONLY) ensureCertificate();
...
const server = HTTP_ONLY
  ? httpServer
  : createHttpHttpsMuxServer(httpServer, tls.createSecureContext({
...
server.listen(PORT, HOST, () => {
  ...
  const urls = HTTP_ONLY
    ? [httpPrimary, ...LAN_IPS.map(ip => `http://${ip}:${PORT}/`)]
    : [httpPrimary, httpsPrimary, ...LAN_IPS.flatMap((ip) => [`http://${ip}:${PORT}/`, `https://${ip}:${PORT}/`])];
  console.log(`${HTTP_ONLY ? 'HTTP' : 'HTTP/HTTPS'} preview serving ${displayPath(SERVE_ROOT)}`);
...
function ensureCertificate() {
  ensureCertificateImpl({
    ...
    localHostname: LOCAL_HOSTNAME,
    lanIps: LAN_IPS,
    opensslPath: resolveOpenSslExecutablePath(),
  });
}
