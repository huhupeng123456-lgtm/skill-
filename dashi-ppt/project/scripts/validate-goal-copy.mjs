...
const htmlPath = path.resolve(CALLER_CWD, htmlArg);
const spec = JSON.parse(readFileSync(specPath, 'utf8'));
const html = readFileSync(htmlPath, 'utf8');
const telemetry = beginWorkflowStage({
  goalPath: specPath,
  telemetryFile: workflowTelemetryPath(specPath),
  stage: 'validate-goal-copy',
  kind: 'validate',
  command: 'validate:goal-copy',
});
...
if (errors.length) {
  telemetry.finish({
    ...
    metrics: { goalSha256: sha256File(specPath), htmlSha256: sha256File(htmlPath) },
  });
