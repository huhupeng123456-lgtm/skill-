...
/**
 * Return the deterministic semantic family used by both layout ranking and
 * goal-quality validation.  This intentionally relies only on inspected
 * layout metadata and the projection plan; it does not infer meaning from
 * authored business copy.
 */
export function layoutSemanticFamily(layout, projectionPlan = {}) {
  if (!layout || typeof layout !== 'object') return 'editorial';
  return layoutStructureFingerprint(layout, projectionPlan).family;
}
