...
长 deck 先写逐页 brief JSON,每页包含 `role`、`priority` 和唯一 `content`,再用 `npm --prefix <skill-root>/project run goal:scaffold -- --title <title> --goal <goal> --theme <themePack> --pages <n> --content-briefs <briefs.json> --layout-variants 3 --seed <randomSeed> --workflow-run-id <workflowRunId> --chunk-size 5 --out output/<deck-name>/goal.json`;有内容来源时追加 `--source-bound`,有绑定外部参考稿时追加 `--reference-bound` 并接受当前运行时的阻塞判定。新任务生成新的 run ID,同一任务重试复用原 ID。scaffold 按内容容量和字段能力选 3 个模板,并追加可直接渲染的主题化 v4 初稿;Agent 保留 compositionFamily 和主题 recipe 做优化,不要清空后重做同一套卡片。输出目录写在当前会话工作目录,不要写入 `<skill-root>/project/output`。
