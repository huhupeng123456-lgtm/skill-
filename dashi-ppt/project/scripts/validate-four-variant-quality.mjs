...
const provenanceState = buildProvenance({ goalPath, deckFile });
const provenance = provenanceState.provenance;
const telemetry = goalPath
  ? beginWorkflowStage({
      goalPath,
...
let report;
try {
  if (provenanceState.errors.length > 0) {
    throw new Error(`Provenance validation failed: ${provenanceState.errors.join('; ')}`);
  }
  ...
  if (!failed && provenance.engine === 'dashi-ppt') {
    report.provenance.identity = 'pre-export-verified';
  }
  ...
  telemetry.finish({
    ...
    metrics: {
      goalSha256: provenance.goalSha256,
      htmlSha256: provenance.deckSha256,
      buildManifestSha256: provenance.buildManifestSha256,
      qualityReportSha256: sha256File(outFile),
    },
  });
  if (failed) {
  ...
  report ||= {
    ...
    generatedAt: new Date().toISOString(),
    provenance,
    ...(provenanceState.errors.length > 0
      ? { provenanceErrors: provenanceState.errors }
      : {}),
    summary: {
...
function buildProvenance({ goalPath: provenanceGoalPath, deckFile: provenanceDeckFile }) {
  const state = verifyBuildChain({
    goalPath: provenanceGoalPath,
    deckFile: provenanceDeckFile,
    requireQuality: false,
  });
  const errors = [...state.errors];
  if (state.strict && state.goal) {
    errors.push(...validateGoalSpec(state.goal).map(error => `goal spec: ${error}`));
  }
  ...
  return {
    ...
    errors,
    provenance: {
      engine: state.strict ? 'dashi-ppt' : null,
      identity: state.identity,
      workflowRunId: state.workflowRunId,
      goalSha256: state.goalSha256,
      deckSha256: state.htmlSha256,
