...
async function main() {
  ...
  const deckRoot = path.basename(deckDir).toLowerCase() === 'ppt' ? path.dirname(deckDir) : deckDir;
  const goalPath = path.join(deckRoot, 'goal.json');
  let chain = verifyBuildChain({
    goalPath: existsSync(goalPath) ? goalPath : null,
    deckFile: indexHtml,
    requireQuality: true,
  });
  if (chain.strict && chain.errors.length) {
    throw new Error(`Verified Dashi export blocked: ${chain.errors.join('; ')}`);
  }
  const telemetry = existsSync(goalPath)
    ? beginWorkflowStage({
        goalPath,
        telemetryFile: workflowTelemetryPath(goalPath),
        stage: pdfMode ? 'export-pdf' : 'export-pptx',
        kind: 'export',
        command: pdfMode ? 'export:pdf' : 'export:pptx',
      })
    : { finish() {} };
  ...
  const server = spawn(process.execPath, [path.join(ROOT, 'scripts/serve-preview-https.mjs'), deckDir, String(port)], {
    ...
    env: {
      ...
      PORT: String(port),
      HOST: '127.0.0.1',
      DASHI_PPT_HTTP_ONLY: '1',
      DASHI_PPT_SKIP_FRESHNESS: '1',
      DASHI_PPT_READ_ONLY: '1',
    },
  ...
  try {
    ...
    const postExportChain = verifyBuildChain({
      goalPath: existsSync(goalPath) ? goalPath : null,
      deckFile: indexHtml,
      requireQuality: true,
    });
    if (postExportChain.strict && postExportChain.errors.length) {
      throw new Error(`Verified Dashi export changed its validated inputs: ${postExportChain.errors.join('; ')}`);
    }
    chain = postExportChain;
    ...
    const exportRecord = writeExportManifest({
      ...
      format: pdfMode ? 'pdf' : 'pptx',
      exporterVersion,
    });
    telemetry.finish({
      ok: true,
      metrics: {
        goalSha256: chain.goalSha256,
        htmlSha256: chain.htmlSha256,
