...
const telemetry = existsSync(goalPath)
  ? beginWorkflowStage({
      ...
      stage: 'validate-swiss',
      kind: 'validate',
      command: 'validate:swiss',
    })
  : { finish() {} };
...
if (errors.length) {
  telemetry.finish({ ok: false, error: new Error(errors.join('; ')), metrics: { htmlSha256: sha256File(file) } });
  console.error('Swiss deck validation failed:');
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

telemetry.finish({ ok: true, metrics: { htmlSha256: sha256File(file) } });
