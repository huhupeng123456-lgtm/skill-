...
if [[ $# -lt 2 || $# -gt 4 ]]; then
  echo "Usage: render_goal_deck.sh <goal-spec.json> <output/ppt/index.html> [--source-bound] [--reference-bound]" >&2
  exit 2
fi
...
for flag in "$@"; do
  case "$flag" in
    --source-bound) CONTRACT_ARGS+=(--require-source-bound) ;;
    --reference-bound) CONTRACT_ARGS+=(--require-reference-bound) ;;
...
npm run validate:goal-spec -- --goal "$SPEC_PATH" "${CONTRACT_ARGS[@]}"
...
npm run render:goal -- "$SPEC_PATH" "$OUT_PATH" "${CONTRACT_ARGS[@]}"
