...
function run() {
  ...
  const briefBundle = parseBriefs(args.briefs || args['content-briefs']);
  const briefs = briefBundle.slides;
  const sourceBound = Boolean(args.sourceBound || args['source-bound'] || briefBundle.sourceLedger);
  const referenceBound = Boolean(args.referenceBound || args['reference-bound'] || briefBundle.referenceBinding);
  ...
  if (sourceBound && !briefBundle.sourceLedger) {
    throw new Error('Source-bound scaffold requires sourceLedger in --content-briefs');
  }
  if (referenceBound) {
    throw new Error('REFERENCE_BINDING_UNSUPPORTED: current Dashi runtime cannot materialize an external deck as a registered theme; stop instead of approximating with theme01-theme12');
  }
  ...
  const spec = {
    ...
    contentMode: sourceBound ? 'source-bound' : 'creative',
    designMode: referenceBound ? 'reference-bound' : 'preset',
    ...(briefBundle.sourceLedger ? { sourceLedger: briefBundle.sourceLedger } : {}),
    ...(briefBundle.referenceBinding ? { referenceBinding: briefBundle.referenceBinding } : {}),
    slides,
  };
...
function buildFourVariantSlides({
  ...
  return descriptors.map((descriptor, index) => {
    ...
    return {
      id: String(descriptor.brief?.id || `s${index + 1}`),
      content: descriptor.content,
      selectedVariant: 'v1',
      variants: [
        ...variants,
...
function parseBriefs(value) {
  if (!value || value === true) return { slides: [], sourceLedger: null, referenceBinding: null };
  const source = String(value).trim();
  const parsed = JSON.parse(source.startsWith('[') || source.startsWith('{')
    ? source
    : readFileSync(path.resolve(CALLER_CWD, source), 'utf8'));
  if (Array.isArray(parsed)) return { slides: parsed, sourceLedger: null, referenceBinding: null };
  return {
    slides: Array.isArray(parsed?.slides) ? parsed.slides : [],
    sourceLedger: parsed?.sourceLedger && typeof parsed.sourceLedger === 'object'
      ? parsed.sourceLedger
      : null,
    referenceBinding: parsed?.referenceBinding && typeof parsed.referenceBinding === 'object'
      ? parsed.referenceBinding
      : null,
  };
}
...
function canonicalSlideContent(brief, slideIndex = 0) {
  ...
  const explicitProvenance = authored.provenance && typeof authored.provenance === 'object'
    && !Array.isArray(authored.provenance)
    ? authored.provenance
    : null;
  const hasBriefProvenance = [
    brief?.pageQuestion,
    brief?.sectionId,
    brief?.claimIds,
    brief?.fieldRefs,
  ].some(value => value !== undefined);
  const provenance = explicitProvenance || hasBriefProvenance
    ? {
        ...(explicitProvenance || {}),
        ...(explicitProvenance?.pageQuestion == null && brief?.pageQuestion != null
          ? { pageQuestion: String(brief.pageQuestion) }
          : {}),
        ...(explicitProvenance?.sectionId == null && brief?.sectionId != null
          ? { sectionId: String(brief.sectionId) }
          : {}),
        ...(explicitProvenance?.claimIds == null && brief?.claimIds != null
          ? { claimIds: brief.claimIds }
          : {}),
        ...(explicitProvenance?.fieldRefs == null && brief?.fieldRefs != null
          ? { fieldRefs: brief.fieldRefs }
          : {}),
        ...(explicitProvenance?.role == null && brief?.role != null
          ? { role: String(brief.role) }
          : {}),
        ...(explicitProvenance?.priority == null && brief?.priority != null
  ...
  return {
    presentation: {
      ...
      items: [],
      ...canonicalPresentation,
      ...(canonicalPresentation.structure == null && brief?.structure != null
        ? { structure: String(brief.structure) }
        : {}),
      coverLabels,
      items: authoredItems,
      chartData,
...
function writeChunks(out, spec, chunkSize) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const chunkSpec = {
      ...
      ...(spec.artifactContract ? { artifactContract: spec.artifactContract } : {}),
      ...(spec.contentMode ? { contentMode: spec.contentMode } : {}),
      ...(spec.designMode ? { designMode: spec.designMode } : {}),
      ...(spec.sourceLedger ? { sourceLedger: spec.sourceLedger } : {}),
      ...(spec.referenceBinding ? { referenceBinding: spec.referenceBinding } : {}),
      part: {
        index: index + 1,
        total,
        startSlide: start + 1,
...
function printUsage() {
  console.error('Usage: node scripts/goal-scaffold.mjs --title <title> --goal <goal> --theme <themeXX> --pages <n> --out output/<deck>/goal.json [--source-bound] [--reference-bound] [--workflow-run-id <id>] [--layout-variants 1|3] [--roles <one-body-role-per-page>] [--chunk-size 5]');
}
